...
class ActaAsambleaForm(forms.ModelForm):
    ...
    orden_dia_texto = forms.CharField(
        ...
        label='Orden del Día',
        help_text='Puntos del orden del día (uno por línea)'
    )
...
class ActaSesionConsejoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'razon_social', 'fecha', 'hora_inicio', 'hora_cierre', 'lugar',
            'convocatoria_realizada', 'porcentaje_miembros_presentes', 'metodo_instalacion',
            'presidente', 'secretario', 'estado'
        ]
        widgets = {
            'fecha': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}),
            'hora_inicio': forms.TimeInput(attrs={'type': 'time', 'class': 'form-control'}),
            'hora_cierre': forms.TimeInput(attrs={'type': 'time', 'class': 'form-control'}),
            'lugar': forms.Textarea(attrs={'rows': 3, 'class': 'form-control'}),
            'porcentaje_miembros_presentes': forms.NumberInput(attrs={
                'class': 'form-control',
                'step': '0.001',
                'min': '0',
                'max': '100'
            ...
            'metodo_instalacion': forms.Select(attrs={'class': 'form-select'}),
            'estado': forms.Select(attrs={'class': 'form-select'}),
        }
...
class PagareForm(forms.ModelForm):
    class Meta:
        ...
        labels = {
            ...
            'moneda': 'Moneda',
            'concepto': 'Concepto del Pagaré',
            'tipo_pago': 'Tipo de Pago',
            'num_pagos': 'Número de Pagos',
            'periodicidad': 'Periodicidad',
            'lugar_pago': 'Lugar de Pago',
            'forma_pago': 'Forma de Pago',
            'tasa_interes_ordinario': 'Tasa de Interés Ordinario (%)',
            'base_intereses': 'Base de Cálculo (días)',
            'tasa_interes_moratorio': 'Tasa de Interés Moratorio (%)',
            'gastos_admon': 'Gastos de Administración',
            'prepago_permitido': 'Permitir Prepago',
            'dias_aviso_prepago': 'Días de Aviso para Prepago',
            'condicion_prepago': 'Condición de Prepago',
            'tiene_garantia': 'Tiene Garantía o Aval',
            'descripcion_garantia': 'Descripción de la Garantía',
            'aval_nombre': 'Nombre del Aval',
            'aval_domicilio': 'Domicilio del Aval',
